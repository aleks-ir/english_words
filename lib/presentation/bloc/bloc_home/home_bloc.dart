...
import 'package:word_study_puzzle/common/constants/letters.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  void _addExploringWord() async {
    currentHistory.wordExploringCount += 1;
    _updateHistory();
  }

  void _addRepeatingWord() async {
    currentHistory.wordRepeatingCount += 1;
    _updateHistory();
  }

  void _setAward() async {
    settings.flameCount += 1;
    currentHistory.awardWasReceived = true;
    _updateHistory();
  ...
  void _updateHistory() async {
    await createUpdateHistoryUsecase(History(
        ...
        awardWasReceived: currentHistory.awardWasReceived));
  }
  ...
  Future<HomeState> _fetchUnexploredWord() async {
  ...
  void _removeLetter(Word word, int letterIndex) {
    ...
    final letter = unexploredWords[wordIndex].selectedLetterList[letterIndex];
  ...
  Map<int, String> _buildLetterMap(String word) {
    ...
    for (var i = 0; i < letterCount; i++) {
      if (i < word.length) {
        ...
        letterMap[randomIndexList[i]] =
            letters.elementAt(_random.nextInt(letters.length - 1));
      }
    }
    return letterMap;
  }
